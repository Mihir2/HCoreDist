...
public class BIFReader extends BayesNet implements TechnicalInformationHandler {
  ...
  public String globalInfo() {
    return "Builds a description of a Bayes Net classifier stored in XML "
  ...
  public BIFReader processString(String sStr) throws Exception {
    ...
    Document doc = factory.newDocumentBuilder().parse(
      new org.xml.sax.InputSource(new StringReader(sStr)));
    doc.normalize();
  ...
  public TechnicalInformation getTechnicalInformation() {
    ...
    result.setValue(Field.AUTHOR,
      "Fabio Cozman and Marek Druzdzel and Daniel Garcia");
   ...
   * parameter and specifies the the network structure and probability tables.
  ...
  public void Sync(BayesNet other) throws Exception {
    ...
    if (nAtts != other.m_Instances.numAttributes()) {
      throw new Exception(
        "Cannot synchronize networks: different number of attributes.");
    }
    ...
    try {
      ...
      while (i < nNodes) {
        ...
        if (i < nNodes) {
          ...
          for (int iNode = 0; iNode < nNodes; iNode++) {
            int iCPT = 0;
            for (int iParent = 0; iParent < m_ParentSets[iNode]
              .getNrOfParents(); iParent++) {
          ...
          for (int iNode = 0; iNode < nNodes; iNode++) {
            int iCPT = 0;
            for (int iParent = 0; iParent < other.getParentSet(m_order[iNode])
              .getNrOfParents(); iParent++) {
              int nParent = m_order[other.getParentSet(m_order[iNode])
                .getParent(iParent)];
              iCPT = iCPT * nCard[nParent] + x[nParent];
            }
            Q = Q
              * other.m_Distributions[m_order[iNode]][iCPT]
                .getProbability(x[iNode]);
          }
