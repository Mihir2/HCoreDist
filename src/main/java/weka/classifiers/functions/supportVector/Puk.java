...
/**
 * <!-- globalinfo-start --> The Pearson VII function-based universal kernel.<br/>
 * <br/>
...
public class Puk extends CachedKernel implements TechnicalInformationHandler {
  ...
  protected double evaluate(int id1, int id2, Instance inst1) throws Exception {
    ...
    if (id1 == id2) {
      ...
      double squaredDifference = -2.0 * dotProd(inst1, inst2) + precalc1
        + m_kernelPrecalc[id2];
      double intermediate = m_factor * Math.sqrt(squaredDifference);
      double result = 1.0 / Math.pow(1.0 + intermediate * intermediate,
        getOmega());
      return result;
    }
  }
