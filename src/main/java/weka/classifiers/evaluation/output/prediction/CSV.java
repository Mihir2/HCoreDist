...
/**
 * <!-- globalinfo-start --> Outputs the predictions as CSV.
 * <p/>
 * <!-- globalinfo-end -->
 * 
...
public class CSV extends AbstractOutput {
  ...
  protected void doPrintHeader() {
    if (m_Header.classAttribute().isNominal()) {
      if (m_OutputDistribution) {
        append("inst#" + m_Delimiter + "actual" + m_Delimiter + "predicted"
          + m_Delimiter + "error" + m_Delimiter + "distribution");
        for (int i = 1; i < m_Header.classAttribute().numValues(); i++) {
          append(m_Delimiter);
        }
      } else {
        append("inst#" + m_Delimiter + "actual" + m_Delimiter + "predicted"
          + m_Delimiter + "error" + m_Delimiter + "prediction");
      }
    } else {
      append("inst#" + m_Delimiter + "actual" + m_Delimiter + "predicted"
        + m_Delimiter + "error");
   ...
   */
  @Override
  protected void doPrintClassification(double[] dist, Instance inst, int index)
    throws Exception {
    ...
    if (inst.dataset().classAttribute().isNumeric()) {
      ...
      // actual
      append(m_Delimiter + ((int) inst.classValue() + 1) + ":"
        + inst.toString(inst.classIndex()));
      // predicted
      if (Utils.isMissingValue(predValue)) {
        append(m_Delimiter + "?");
      } else {
        append(m_Delimiter + ((int) predValue + 1) + ":"
