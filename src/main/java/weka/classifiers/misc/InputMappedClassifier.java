...
/**
 * <!-- globalinfo-start --> Wrapper classifier that addresses incompatible
...
public class InputMappedClassifier extends SingleClassifierEnhancer implements
  ...
  public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(new Option(
      "\tSuppress the output of the mapping report.", "M", 0, "-M"));
    newVector.addElement(new Option(
      "\tTrim white space from either end of names " + "before matching.",
      "trim", 0, "-trim"));
    newVector.addElement(new Option(
      "\tPath to a model to load. If set, this model"
        + "\n\twill be used for prediction and any base classifier"
        + "\n\tspecification will be ignored. Environment variables"
        + "\n\tmay be used in the path (e.g. ${HOME}/myModel.model)", "L", 1,
      "-L <path to model to load>"));
  ...
  /**
   * Set the test structure (if known in advance) that we are likely to see. If
   * set, then a call to buildClassifier() will not overwrite any test structure
   * that has been recorded with the current training structure. This is useful
   * for getting a correct mapping report output in toString() after
   * buildClassifier has been called and before any test instance has been seen.
   * Test structure and mapping will get reset if a test instance is received
   * whose structure does not match the recorded test structure.
   * 
   * @param testStructure the structure of the test instances that we are likely
   *          to see (if known in advance)
   */
  ...
  /**
   * Set the structure of the data used to create the model. This method is
   * useful for clients who have an existing in-memory model that they'd like to
   * wrap in the InputMappedClassifier
   * 
      ...
      for (int j = 0; j < m_inputHeader.numAttributes(); j++) {
        ...
        if (stringMatch(modelAttName, incomingAttName)) {
          ...
          if (modelAtt.numValues() != incomingAtt.numValues()) {
            System.out
              .println("[InputMappedClassifier] Warning: incoming nominal "
                + "attribute " + incomingAttName + " does not have the same "
                + "number of values as model attribute " + modelAttName);
   ...
   * this method just returns the default structure that is passed in as
   * argument.
   * 
    ...
    for (int i = 0; i < m_modelHeader.numAttributes(); i++) {
      if (m_attributeStatus[i] == OK) {
        ...
        if (modelAtt.isNumeric()) {
          ...
          int mapVal = m_nominalValueMap[i][(int) incoming
            .value(m_attributeMap[i])];
