...
public class HierarchicalClusterer extends AbstractClusterer implements
	...
	public HierarchicalClusterer(String DistanceFunctionName, int numClusters) {
		setNumClusters(numClusters);
		if (DistanceFunctionName == "Manhattan") {
			m_DistanceFunction = new ManhattanDistance();

		} else if (DistanceFunctionName == "Euclidean") {
...
//	public HierarchicalClusterer(boolean ManhattanDistanceflag, int numClusters, String attrIn) {
//		setNumClusters(numClusters);
...
//	}
	...
	public static final Tag[] TAGS_LINK_TYPE = { new Tag(SINGLE, "SINGLE"), new Tag(COMPLETE, "COMPLETE"),
			new Tag(AVERAGE, "AVERAGE"), new Tag(MEAN, "MEAN"), new Tag(CENTROID, "CENTROID"), new Tag(WARD, "WARD"),
			new Tag(ADJCOMPLETE, "ADJCOMPLETE"), new Tag(NEIGHBOR_JOINING, "NEIGHBOR_JOINING") };
	...
	class Node implements Serializable {
		...
		public String toString2(int attIndex) {
			...
			if (m_left == null) {
				if (m_right == null) {
					tot = tot + m_instances.instance(m_iLeftInstance) + "\n";
					tot = tot + m_instances.instance(m_iRightInstance) + "\n";
	...
	public Enumeration<Option> listOptions() {
		...
		newVector.addElement(
				new Option("\tIf set, distance is interpreted as branch length\n" + "\totherwise it is node height.",
						"B", 0, "-B"));
		...
		newVector.addElement(new Option(
				"Link type (Single, Complete, Average, Mean, Centroid, Ward, Adjusted complete, Neighbor joining)", "L",
				1, "-L [SINGLE|COMPLETE|AVERAGE|MEAN|CENTROID|WARD|ADJCOMPLETE|NEIGHBOR_JOINING]"));
	...
	public String[] getOptions() {
		...
		options.add("-A");
		options.add((m_DistanceFunction.getClass().getName() + " " + Utils.joinOptions(m_DistanceFunction.getOptions()))
				.trim());
	...
	public static void main(String[] argv) {
		runClusterer(new HierarchicalClusterer("Euclidean", 1), argv);
	}
...
//		tot = "";
		if(m_clusters[i] == null){
			sNewick += "Newick" + i + ": Not Available\n";
			continue;
		}
    	if (m_instances.attribute(attIndex).isString()) {
    	      sNewick += "Newick" + i + ": " + m_clusters[i].toString(attIndex) + "\n";
    	    } else {
    	      sNewick += "Newick" + i + ": " + m_clusters[i].toString2(attIndex) + "\n";
    	    }
//    	tot = attr + tot;
...
//    	Instances filteredClusteredData = clusterData;
//    	StringToNominal stn;
//    	try{
//    		for (int attIndex1=0; attIndex1 < clusterData.numAttributes() - 1; attIndex1++) {
//    		      if (clusterData.attribute(attIndex1).isString()) {
//    		        stn = new StringToNominal();
//    		        stn.setInputFormat(filteredClusteredData);
//    		        filteredClusteredData = Filter.useFilter(filteredClusteredData,stn);
//    		        System.out.println(filteredClusteredData);
//    		      }
//    		    }
//    	}
//    	catch(Exception e){
...
//		kmeans.buildClusterer(filteredClusteredData);
//		Instances Centroid = kmeans.getClusterCentroids();
//		CentroidStr = CentroidStr + Centroid.firstInstance().toString() + "\n";
    }
    return sNewick;
  }
